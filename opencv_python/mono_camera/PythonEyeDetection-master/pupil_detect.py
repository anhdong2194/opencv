...
def find_average(image, feature):
    feature_image = mask_image(image, feature)
...
def sort_features_by_brightness(image, features):
    features_and_brightnesses = [(find_average(image, feature), feature) for feature in features]
    features_and_brightnesses.sort(key = lambda x:x[0])
    return [fb[1] for fb in features_and_brightnesses]

def draw_circle(image, feature, color=255, thickness=1):
...
def find_pupil(gray_image, minsize=.1, maxsize=.5):
    detector = cv2.MSER_create()#FeatureDetector_create('MSER')
...
def main():
    ...
    while(1):
        ...
        photo = cv2.imread("images/eyes/"+eye)
        #photo = cv2.imread("/home/dongho/Desktop/mono_camera/PythonEyeDetection-master/images/eyes/S1001L01.jpg")
        time.sleep(3)
        if draw(photo) > 0:
            break
